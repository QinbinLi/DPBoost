...
//    std::cout<<"in serial tree global iter:"<<global_iter<<std::endl;
    ...
    double lambda = 0.1;
    double sum_leave_sensitivity = change_round * std::pow(g_m / (1+lambda), 2.0/3.0) + std::pow(2*std::pow(base,change_round),2.0/3.0) * (1-std::pow(base,2.0*(total_iter-change_round))) / (1-std::pow(base, 2.0/3.0));
    double sensitivity_this_tree;
    if(global_iter + 1 < change_round)
      sensitivity_this_tree = g_m / (1 + lambda);
    else
      sensitivity_this_tree = (2.0 * std::pow(base, global_iter));

    double budget_for_this_tree = total_budget * sensitivity_this_tree / sum_leave_sensitivity;
...
//    double budget_for_current_node = budget_for_this_tree /2 / 6;
...
#pragma omp parallel for
    for(int i = 0; i < num_features_; i++) {
        probs[i].resize(gains_of_features_of_leaves[best_leaf][i].size(), 0);
...
//                            std::cout<<"-nan_gain_diff"<<std::endl;
                          sum += std::exp(gains_of_features_of_leaves[best_leaf][m][n] - gains_of_features_of_leaves[best_leaf][i][j]);
                          if(sum == INFINITY){
                              probs[i][j] = 0;
                              is_continue = false;
                          }
